...
app.use(express.json({ extended: true })); // Use 'extended' option (already addressed)
app.use(express.urlencoded({ extended: true })); // Add this for URL-encoded data
...
app.post('/addFile', upload.single('file'), async (req, res) => {
  try {
    ...
    const pinnedFile = await ipfs.pin.add(cid);
    console.log('File pinned:', pinnedFile);
